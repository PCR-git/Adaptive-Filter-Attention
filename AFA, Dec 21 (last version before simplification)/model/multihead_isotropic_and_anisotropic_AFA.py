...
def compute_attention_matrix(self, R_qk_abs_squared, V_ij_k, V_ij_v, V_tilde, Delta_T, nu, args):
    ...
    if args.additive_bias_type == 0:
        print('Using zero additive bias.')
        B = 0 # Zero bias
    elif args.additive_bias_type == 1:
        B = - torch.log(V_ij_v + args.epsilon) # DLE Bias
#         print('Using DLE additive bias.')
    elif args.additive_bias_type == 2:
        B = - nu * Delta_T # Linear bias
        print('Using linear additive bias.')
    else:
        print('Error: Additive bias must be 0 (zero), 1 (DLE), or 2 (Linear)')
        
    if args.multiplicative_bias_type == 0:
        S = d_head # Constant bias
        print('Using constant multiplicative bias.')
    elif args.multiplicative_bias_type == 1:
        S = nu * d_head * V_ij_k # DLE Bias
#         print('Using DLE multiplicative bias.')
    elif args.multiplicative_bias_type == 2:
        S = nu * d_head * Delta_T # Linear bias
        print('Using linear multiplicative bias.')
    else:
        print('Error: Multiplicative bias must be 0 (const), 1 (DLE), or 2 (Linear)')

    if args.use_full_residual_norm == 1:
        ...
        print('Using exponential robust weight (rather than rational weight).')

    attention_scores = self.tau**2 * base_attn_scores
    
    causal_mask = torch.tril(torch.ones(L_seq, L_seq, device=args.device)).view(1, L_seq, L_seq, 1)

    attention_scores_masked = attention_scores.masked_fill(causal_mask == 0, float('-inf')) # Set to -infinity where mask is 0
    ...
    A_prior = (1/(V_ij_v + args.epsilon)).masked_fill(causal_mask == 0, float('0')).squeeze(0)
...
#             self.lambda_imag_v = nn.Parameter(omega_init_stacked * (1 + 0.5*rand_noise)) # RoPE initialization + multiplicative rand noise
        ...
        if self.args.isotropic_afa == 1:
            self.lambda_real_v = nn.Parameter(torch.randn(self.n_heads))
        else:
            rand_noise = torch.randn(self.n_heads, dim_target)
            self.lambda_real_v = nn.Parameter(rand_noise) # Gaussian noise
        ...
        if self.args.sep_params == 1:
            ...
            if self.args.isotropic_afa == 1:
                ...
                rand_noise = torch.randn(self.n_heads, dim_target)
                self.lambda_real_k = nn.Parameter(rand_noise) # Gaussian noise
#                 self.lambda_imag_k = nn.Parameter(omega_init_stacked) # Initialize as in RoPE
#                 self.lambda_imag_k = nn.Parameter(omega_init_stacked * (1 + 0.5*rand_noise)) # RoPE initialization + multiplicative rand noise
            ##########################
            ...
            if self.args.learn_rotations == 1:
                rand_noise = torch.randn(self.n_heads, dim_target)
                self.lambda_imag_k = nn.Parameter(rand_noise) # Gaussian noise
#                 print('Learned complex rotations.')
            else:
...
#                 print('Fixed complex rotations.')
        ...
        if self.args.use_complex_input_norm == 1:
            self.n_q = ComplexRMSNorm(self.d_k_total, self.n_heads)
            self.n_k = ComplexRMSNorm(self.d_k_total, self.n_heads)
            self.n_v = ComplexRMSNorm(self.d_v_total, self.n_heads)
        else:
            pass

        if self.args.use_complex_output_norm == 1:
            self.n_o = ComplexRMSNorm(self.d_v_total, self.n_heads)
        else:
            pass
        ...
        self.g1_param = nn.Parameter(torch.zeros(1, 1, 1, self.d_v_head, self.n_heads))
        self.g2_param = nn.Parameter(torch.zeros(1, 1, 1, self.d_v_head, self.n_heads))
...
#         t_measure = t_measure / (t_measure[-1] - t_measure[0]).unsqueeze(0)
        ...
        if self.args.use_complex_input_norm == 1:
            ...
            K = self.n_k(K)
            V = self.n_v(V)
#             print('Using complex input norm.')
        else:
#             print('Using real input norm.')
            pass
...
#             print('Using Isotropic AFA.')
            
        else:
            Phi_hat_plus_v, Phi_hat_minus_v = compute_exp_kernel_anisotropic(t_measure, lambda_v, self.args)
#             print('Using Anisotropic AFA.')
...
#         V_ij_v = compute_covariance_matrix(mu_v, sigma_squared_v, eta_squared_v, gamma_squared_v, E_rel_v, self.args)
        ...
        if self.args.sep_params == 1:
            ...
            if self.args.isotropic_afa == 1:
                ...
                Phi_hat_plus_k, Phi_hat_minus_k = compute_exp_kernel_anisotropic(t_measure, lambda_k, self.args)
...
#             V_ij_k = compute_covariance_matrix(mu_k, sigma_squared_k, eta_squared_k, gamma_squared_k, E_rel_k, self.args)                
            ...
            if self.args.isotropic_afa == 1:
                ...
                E_rel_k = E_rel_v
#                 Phi_hat_plus_k = Phi_hat_plus_v #### DELETE
#                 Phi_hat_minus_k = Phi_hat_minus_v #### DELETE
            else:  
                Phi_hat_plus_k = Phi_hat_plus_v
                Phi_hat_minus_k = Phi_hat_minus_v
        
        if self.args.isotropic_afa == 1:
            Q_tilde, K_tilde, V_tilde = build_factorized_kernels(Phi_tilde_minus_k, Phi_tilde_minus_v, Q, K, V, self.args)
            R_qk_abs_squared = compute_residual_norm_isotropic(Q, Q_tilde, K_tilde, E_rel_k, self.args)
            ...
            _, K_hat, V_hat = build_factorized_kernels(Phi_hat_minus_k, Phi_hat_minus_v, Q, K, V, self.args)
            R_qk_abs_squared = compute_residual_norm_anisotropic(Q, K_hat, Phi_hat_plus_k, self.args)
            A, A_prior, unnormalized_attention = compute_attention_matrix(self, R_qk_abs_squared, V_ij_k, V_ij_v, V_hat, Delta_T, nu, self.args)
            est_v = compute_estimate(A, V_hat, Phi_hat_plus_v, self.args)

        # Add residual connection
        if self.args.inner_residual == 1:
#             print('Using inner residual.')
            if self.args.use_total_precision_gate == 0:
#                 print('Not using total precision gating.')
                g1 = torch.sigmoid(self.g1_param) # Learned gate
                est_latent = g1 * est_v + (1-g1) * V # Update using convex combination
            else:
#                 print('Using total precision gating.')
                P_tot = torch.sum(unnormalized_attention,-2).unsqueeze(1).unsqueeze(-2) # Sum precision
                g = torch.sigmoid(P_tot * self.g1_param + self.g2_param) # Mean precision gate
                est_latent = g * est_v + (1-g) * V # Update using convex combination
        else:
            est_latent = est_v # No residual
#             print('Not using inner residual.')

        # Use complex normalization on outputs
        if self.args.use_complex_output_norm == 1:
            est_latent = self.n_o(est_latent)
#             print('Using complex output norm.')
        else:
#             print('Using real output norm.')
            pass
        ...
        out = self.W_o(pred_p_reshape)[:,0].unsqueeze(1)
        ...
        if self.args.isotropic_afa == 1:
            ...
            Z_ij_hat_all_multihead = batched_complex_hadamard_full(Phi_hat_plus_v.unsqueeze(0).unsqueeze(3), V_hat.unsqueeze(2))
        ...
        causal_mask = torch.tril(torch.ones(Z_ij_hat_all.size()[2], Z_ij_hat_all.size()[3])).view(1,1,Z_ij_hat_all.size()[2],Z_ij_hat_all.size()[3],1).to(self.args.device)
        Z_ij_hat_causal = Z_ij_hat_all * causal_mask
