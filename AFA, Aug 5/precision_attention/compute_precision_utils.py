...
#     mag_f2 = (mag_f**2).unsqueeze(0)
    ...
    exp_b = complex_exp(-lambda_h*t_v.unsqueeze(0).unsqueeze(2).unsqueeze(3))  # Backward

    mag_f2 = torch.sum(exp_f**2, dim=0, keepdims=True)

    exp_c = torch.stack((torch.ones_like(exp_f[0]), torch.zeros_like(exp_f[1]))).to(lambda_h.device) # IF CAUSAL
    mag_c = torch.ones_like(mag_f2).to(lambda_h.device)
#     mat_exp_c = torch.stack((torch.ones(args.seq_len,args.d_v,1), torch.zeros(args.seq_len,args.d_v,1))).to(lambda_h.device) # IF CAUSAL
#     mag_c = torch.ones(1, args.seq_len,args.d_v,1).to(lambda_h.device)

    # Full kernel
    K_exp = torch.concatenate((exp_f.flip(dims=[1])[:,:-1,:,:], exp_c), dim=1)  # Matrix exponential kernel
    K_exp2 = torch.concatenate((mag_f2.flip(dims=[1])[:,:-1,:,:], mag_c), dim=1)  # Matrix exponential kernel

    return K_exp, K_exp2, exp_f, exp_b
...
def batched_compute_estimates_and_residuals_vectorized(X_q, X_k, X_v, K_exp, args):
    ...
    Returns:
      Z_ij_hat_all (torch.Tensor): Estimated states.
    ...
    Z_ij_hat_k = batched_complex_hadamard(mat_exp_slices, X_k.unsqueeze(2))
    Z_ij_hat_v = batched_complex_hadamard(mat_exp_slices, X_v.unsqueeze(2))

    # Compute residuals
    R_qk_ij = X_q.unsqueeze(3) - Z_ij_hat_k

    return Z_ij_hat_v, R_qk_ij
...
def compute_estimates_and_residuals_irregular_times(lambda_h, t_v_all, X_q, X_k, X_v, args):
    ...
    # Compute all estimated states at once
    Z_ij_hat_k = batched_complex_hadamard_full(mat_exp_slices, X_k.unsqueeze(2))
    Z_ij_hat_v = batched_complex_hadamard_full(mat_exp_slices, X_v.unsqueeze(2))
