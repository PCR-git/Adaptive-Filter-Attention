...
class ComplexTransformerBlock(nn.Module):
    ...
    - Two-layer complex feedfo
    rward network
    - Pre-layer normalization
    - Residual connections
    """

    def __init__(self, input_dim, hidden_dim, args, Norm=GatedComplexLayerNorm):
        """
        Args:
            input_dim (int): Dimensionality of input and output.
        ...
        # === Attention block ===
        self.attn = ComplexAttentionLayer(input_dim, hidden_dim, args, Norm=Norm)
...
class ComplexRealTransformerBlock(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, args, Norm=nn.LayerNorm):
        """
        ...
        # Self-attention layer
        self.attn = ComplexAttentionLayer(input_dim, input_dim, args,Norm=Norm)
...
class AFATransformerBlock(nn.Module):
    ...
    def __init__(self, input_dim, query_key_dim, value_dim, num_layers, args, Norm=nn.LayerNorm):
        """
    ...
    def forward(self, x, t_v):
        ...
        # Self-attention with residual connection
        est, attn_out, attn_weight, Z_ij_hat, lambda_h = self.attn(x, x, x, t_v)
        x = attn_out[:,0].unsqueeze(1) # Residual connection is already inside self.attn
#         x = x + attn_out[:,0].unsqueeze(1) # Residual connection
