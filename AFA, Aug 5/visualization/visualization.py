...
from model import ComplexLinearLayer, FullPrecisionAttentionBlock, FullPrecisionAttentionBlockShared, FullPrecisionAttentionBlock_Nlayer, AFATransformerBlock
from model import AFA_1layer, AFA_Nlayer, AFATransformerNetwork
...
#         W_p = module.W_p
        ...
        # Get eigenvalues
#         lambda_h_model = compute_lambda_h(module.lambda1,args)
        lambda_h_model = compute_lambda_shared(self.lambda_real_v, self.lambda_imag_v, self.args)
...
#         est, out, Q_ij, Z_ij_hat_all, epoch_lambdas = model.forward(inputs, inputs, inputs, t_measure.unsqueeze(0))

#         est = est.unsqueeze(-1)
#         out = out.unsqueeze(-1)
#         Z_ij_hat_all = Z_ij_hat_all.unsqueeze(-1)
...
#         x_hat = batched_complex_matmul(W_p, Z_ij_hat_all*module.causal_mask)
...
def visualize_results(model, train_dataset, all_losses, mean_epoch_losses, log_mean_epoch_losses, all_lambdas, R1, R1i, Pu, Pd, A, epoch, args):
    ...
    Prints the following:
        ...
        W_v W_p, which we expect to be about I
    ...
    with torch.no_grad():
        ...
        est, out, Q_ij_tot, Z_ij_hat_all, epoch_lambdas = model.forward(inputs, t_measure.unsqueeze(0))
...
#         x_hat = batched_complex_matmul(W_p, Z_ij_hat_all * module.causal_mask)
#         x_hat = module.W_p(Z_ij_hat_all * module.causal_mask.unsqueeze(0).squeeze(-1)).unsqueeze(-1)

#         print(Z_ij_hat_all.size())
#         print(Z_ij_hat_all.squeeze(-1).size())
#         print(module.causal_mask.unsqueeze(0).squeeze(-1).size())
        x_hat = module.W_p(Z_ij_hat_all * module.causal_mask.unsqueeze(0)).unsqueeze(-1)
...
#         plt.show()
        ...
        for head in range(Q_ij_tot.size()[-1]):
            Q_ij = Q_ij_tot[:,:,:,:,head]
            Q_ij_avg = Q_ij.squeeze()[0].detach().cpu().numpy() # If using simplified model
            plt.imshow(Q_ij_avg**0.25) # (Power is just to increase contrast for better visualization)
    #         plt.savefig(folder + 'attn//' + str(epoch) + '.png', bbox_inches='tight')
            plt.title('Attention Matrix')
            plt.show()
...
#         print(lambda_h.squeeze()[:,0]) # Print eigenvalues
        ...
        #           print('alpha=', module.alpha)
        #           print('beta=', module.beta)
#         print('W_p * W_v: (should be near I:')
#         print(complex_matmul(W_p, W_v).squeeze().detach().cpu()[0][0:args.m,0:args.m]) # Should be near identity
...
def _get_visual_modules(model: nn.Module):
    ...
    # Case 5: Model is AFA_1layer
    if isinstance(model, AFA_1layer):
        if hasattr(model, 'layers') and len(model.layers) > 0 and \
           isinstance(model.layers[0], FullPrecisionAttentionBlock):
            main_attention_block = model.layers[0]
            last_inner_layer_of_main_attention_block = model.layers[0]
            
    # Case 6: Model is SimplifiedAFA_1layer
    if isinstance(model, SimplifiedAFA_1layer):
        if hasattr(model, 'layers') and len(model.layers) > 0 and \
           isinstance(model.layers[0], SimplifiedPrecisionAttentionBlock):
            main_attention_block = model.layers[0]
            last_inner_layer_of_main_attention_block = model.layers[0]
            
    # Case 7: Model is MultiheadSimplifiedAFA_1layer
    if isinstance(model, MultiheadSimplifiedAFA_1layer):
        if hasattr(model, 'layers') and len(model.layers) > 0 and \
           isinstance(model.layers[0], MultiheadSimplifiedPrecisionAttentionBlock):
            main_attention_block = model.layers[0]
            last_inner_layer_of_main_attention_block = model.layers[0]
