...
class TransformerNetwork(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, args, num_blocks=2):
        """
        Args:
...
class ComplexTransformerNetwork(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, args, num_blocks=2):
        ...
        # Final norm layer
        self.final_norm = ComplexGatedLayerNorm(input_dim, args)
...
#         self.a1 = FullPrecisionAttentionBlock(args.d_e, args)
    ...
    FullPrecisionAttentionBlock(args.d_e, args)])

     # Build the network:
    def forward(self, inputs, t_v):
        
        layer = self.layers[0]
        est, out, Q_ij, X_ij_hat_all, lambda_h = layer(inputs, inputs, inputs, t_v)
...
class AFATransformerNetwork(nn.Module):
    ...
    def __init__(self, input_dim, query_key_dim, value_dim, args, num_inner_layers=2, num_blocks=2):
        """
        Initializes the transformer network.
...
#         self.pos_encoder = RealPositionalEncoding(input_dim)
        ...
        # Stack of Transformer blocks, each using complex-valued attention
        self.blocks = nn.ModuleList([      
            AFATransformerBlock(input_dim, query_key_dim, value_dim, num_inner_layers, args)
            for _ in range(num_blocks)
        ])
...
#         x = self.pos_encoder(x)
        ...
        # Sequentially apply each Transformer block
        for block in self.blocks:
            est, attn_out, attn_weight, X_ij_hat, lambda_h = block(x, t_v)
#             attn_list.append(attn_weight)

        # Apply final normalization
        x = self.final_norm(attn_out)
