...
class FullPrecisionAttentionBlock(nn.Module):
    def __init__(self, head_dim, args):
        ...
        self.W_q = ComplexLinearLayer(args.d_e, args.d_k)
        self.W_k = ComplexLinearLayer(args.d_e, args.d_k)
        self.W_v = ComplexLinearLayer(args.d_e, args.d_v)
#         self.W_e = ComplexLinearLayer(args.d_e, args.d_v)
        self.W_p = ComplexLinearLayer(args.d_e, args.d_v)
#         self.W_r = ComplexLinearLayer(args.d_e, args.d_v)
...
#         self.lambda_Gamma = nn.Parameter(torch.randn(1,self.d_v,1)/sqrt_dv) # Measurement covariance (if using **2)
        ...
        if args.sep_params == 1:
            ...
            self.lambda_h_k = torch.zeros(2,self.head_dim,1).to(args.device) # Initialize full eigenvalue array
...
#         Q_ij = A_ij / S_ij # Normalize
        ...
        X_ij_hat_all = X_ij_hat_all * self.causal_mask # Mask out estimates backward in time (not strictly necessary but useful later for visualization)
...
#         est = est.squeeze(-1)
        ...
        return est_latent, out, Q_ij, X_ij_hat_all, lambda_h
