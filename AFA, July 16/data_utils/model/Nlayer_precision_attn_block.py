...
class FullPrecisionAttentionBlockShared(nn.Module):
    ...
    def __init__(self, input_dim, query_key_dim, value_dim, args, shared_params=None):
        super().__init__()
        # self.head_dim = head_dim # Original, now derived from query_key_dim or value_dim depending on context
        ...
        self.lambda_h = torch.zeros(2,self.d_v,1).to(args.device)
        ...
        # Identity matrix for computing covariance, ensures positive definiteness and aids in residual calculations
        ...
        self.gamma_param = nn.Parameter(torch.zeros(1, 1, 1, self.d_v, 1)) # Momentum coeff
        
        # Optional: Separate dynamics and noise parameters specifically for the key projection, for increased model capacity
        ...
        self.causal_mask = torch.tril(torch.ones(args.seq_len, args.seq_len)).view(1, args.seq_len, args.seq_len, 1, 1).to(args.device)
    ...
    def forward(self, Z_q, Z_k, Z_v, t_measure_all, Z_v_prev=None):
        ...
        # Select relevant time steps for kernel computation
        if len(t_measure_all.size()) > 1:
            t_measure = t_measure_all[0, 0:self.args.seq_len]
        else:
            t_measure = t_measure_all[0:self.args.seq_len]
        ...
        if self.args.sep_params == 1:
            ...
            if self.args.t_equal == 1: # For equal time steps
                ...
                K_exp_k, K_exp2_k = compute_exp_kernel(lambda_h_k, t_measure)
                # State estimates and residuals:
                X_ij_hat_all, R_qk_ij = batched_compute_estimates_and_residuals_vectorized(Q, K, V, K_exp_k, self.args)
                mat_exp = K_exp[:, -(self.args.seq_len+1), :, :] # Mat exp for next time step
            else: # For irregular time steps
                X_ij_hat_all, R_qk_ij = compute_estimates_and_residuals_irregular_times(lambda_h_k, t_measure_all[:,:-1], Q, K, V, self.args)
            ...
            if self.args.t_equal == 1: # Equal time steps
                K_exp, K_exp2 = compute_exp_kernel(lambda_h, t_measure) # Exponential kernel
                # State estimates and residuals:
...
#            attention_scores = - torch.log(base_attn_scores)
        ...
        X_ij_hat_all = X_ij_hat_all * self.causal_mask

        # Compute the attention-weighted average of state estimates (z_avg^(k))
        est_v_avg = torch.sum(Q_ij.unsqueeze(1) * X_ij_hat_all,axis=3)
...
#                gamma = torch.sigmoid(self.gamma_param) # Purely learned gamma (commented out)
        ...
        X_ij_hat_all = X_ij_hat_all.squeeze(-1) # Attention-weighted raw estimates

        # Return the refined latent state, attention weights, raw estimates, dynamics, and prediction matrix
        return est_latent, Q_ij, X_ij_hat_all, lambda_h, mat_exp_for_pred
    ...
    def forward(self, Z_q, Z_k, Z_v, t_v):
        ...
        if self.num_inner_layers == 1:
            ...
            for layer_idx, layer in enumerate(self.layers):
                ...
                est_v_next, Q_ij, X_ij_hat_all, lambda_h, current_mat_exp_for_pred = \
            ...
            final_Q, final_X_hat, final_lambda_h, mat_exp_for_pred = \
                Q_ij, X_ij_hat_all, lambda_h, current_mat_exp_for_pred
