...
class GatedComplexSequenceNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        x_rot_centered = x_rot - self.gp1 * x_rot.mean(dim=-2, keepdim=True) # Subtract mean along time dim

        real, imag = x_rot_centered[:, 0], x_rot_centered[:, 1] # Get real and imaginary parts
        mag = torch.sqrt(real**2 + imag**2) # Magnitude
        
        mag_std = mag.std(dim=-2, keepdim=True) # Standard dev of magnitude along time dim
...
class DynamicLayerNorm(nn.Module):
    """
    This layer performs a learned LTI-inspired basis rotation,
    projects data to a t=0 reference frame, applies Layer normalization across features,
    and then projects back to the original time frames and basis.
    It's designed for complex-valued inputs, handled as (real, imag) pairs.
    """
    
    def __init__(self, input_dim, args, shared_input=False, shared_output=False):
        super().__init__()
        
#         # Linear layers
#         self.W_v = ComplexLinearLayer(input_dim, input_dim)
#         self.W_p = ComplexLinearLayer(input_dim, input_dim)
        
#         # Initialize linear layers
#         W1, b1 = init_complex_matrix(input_dim, input_dim, bias=True)
#         W2, b2 = init_complex_matrix(input_dim, input_dim, bias=True)
#         init_complexlinear(self.W_v, W1, b1)
#         init_complexlinear(self.W_p, W2, b2)
        
        # Scale and shift params
        self.alpha_r = nn.Parameter(torch.ones(input_dim)) # Real scale param
        self.alpha_i = nn.Parameter(torch.ones(input_dim)) # Imag scale param
        self.beta_r = nn.Parameter(torch.zeros(input_dim)) # Real shift param
        self.beta_i = nn.Parameter(torch.zeros(input_dim)) # Imag shift param
        
        self.lambda1 = nn.Parameter(torch.randn(2, int(input_dim/2), 1)) # Stack and scale by time interval
#         self.register_buffer("lambda_h", torch.zeros(2,input_dim,1))

        self.lambda_Omega = nn.Parameter(torch.randn(1,input_dim,1)/input_dim) # Process covariance (if using abs)
        self.lambda_Gamma = nn.Parameter(torch.randn(1,input_dim,1)/input_dim) # Measurement covariance (if using abs)
        self.register_buffer("lambda_C", torch.ones(1,input_dim,1))
#         self.register_buffer("lambda_Omega", torch.ones(1,input_dim,1))
#         self.register_buffer("lambda_Gamma", torch.ones(1,input_dim,1))
        
        self.shared_input = shared_input
        self.shared_output = shared_output
        if shared_input == True or shared_output == True:
            self.lambda2_imag = nn.Parameter(torch.randn(int(input_dim/2)))
            self.lambda2_real = nn.Parameter(torch.randn(1))
            self.lambda_Omega2 = nn.Parameter(torch.randn(1)) # Process covariance (if using abs)
            self.lambda_Gamma2 = nn.Parameter(torch.randn(1)) # Measurement covariance (if using 
    
        self.args = args

    def forward(self, x, t_measure, epsilon=1e-5):
        ...
        lambda_Gamma = torch.abs(self.lambda_Gamma) + epsilon # Measurement noise matrix

        exp_f, exp_b, Lambda_v_std = compute_lambda_std(lambda_h, lambda_Omega, lambda_Gamma, self.lambda_C, t_measure, self.args)

        if self.shared_input == True or self.shared_output == True:
            lambda_h_shared = compute_lambda_shared(self.lambda2_real, self.lambda2_imag, self.args)
            lambda_Omega_shared = torch.abs(self.lambda_Omega2)*torch.ones_like(self.lambda_Omega2)
            lambda_Gamma_shared = torch.abs(self.lambda_Gamma2)*torch.ones_like(self.lambda_Omega2) + epsilon

            exp_f_shared, exp_b_shared, Lambda_v_std_shared = compute_lambda_std(lambda_h_shared, lambda_Omega_shared, lambda_Gamma_shared, self.lambda_C, t_measure, self.args)
            
        ######################

        if self.shared_input == True:
            exp_f, exp_b, Lambda_v_std = exp_f_shared, exp_b_shared, Lambda_v_std_shared

#         # Convert real-valued inputs to complex by appending zero imaginary parts, if necessary
#         if x.size()[1] == 1:
#             x = torch.cat((x, torch.zeros_like(x)),dim=1)

#         x_rot = self.W_v(x) # Rotate to new basis    
        x_rot = x
    
        x_hat_0 = batched_complex_hadamard(exp_b, x_rot.unsqueeze(-1)) # Map to t_0 frame
        mu = torch.mean(x_hat_0,axis=2,keepdims=True)
        x_hat_0_shift = x_hat_0 - mu
        log_diff = torch.log(torch.abs(x_hat_0_shift) + epsilon) - torch.log(Lambda_v_std + epsilon)
        x_hat_0_whitened_shift = torch.sign(x_hat_0_shift) * torch.exp(log_diff)
        x_hat_0_whitened = x_hat_0_whitened_shift + mu

        real, imag = x_hat_0_whitened[:, 0], x_hat_0_whitened[:, 1]
        
        mu_real = torch.mean(real, axis=1, keepdims=True)
        mu_imag = torch.mean(imag, axis=1, keepdims=True)

        real_shift = real - mu_real
        imag_shift = imag - mu_imag

        mag2 = real_shift**2 + imag_shift**2 # Magnitude squared

        mag_rms = torch.sqrt(torch.mean(mag2, dim=2, keepdim=True) + epsilon) # Compute RMS along embed dim

        # Apply the scaling to real and imaginary parts based on normalized magnitude
        # This keeps the phase.
        real_norm = real_shift / mag_rms # Normalized magnitude * cosine(phase)
        imag_norm = imag_shift / mag_rms # Normalized magnitude * sine(phase)

        # Scale and shift
        real_out = self.alpha_r * real_norm.squeeze(-1) + mu_real.squeeze(-1) - self.beta_r
        imag_out = self.alpha_i * imag_norm.squeeze(-1) + mu_imag.squeeze(-1) - self.beta_i
        x_hat_0_norm_whitened = torch.stack((real_out, imag_out),dim=1) # Reconstruct full complex vector

        mu_norm = torch.mean(x_hat_0_norm_whitened,axis=2, keepdims=True)
        x_hat_0_norm_whitened_shift = x_hat_0_norm_whitened - mu_norm

        if self.shared_output == True:
            exp_f, exp_b, Lambda_v_std = exp_f_shared, exp_b_shared, Lambda_v_std_shared

        x_hat_0_norm_shift = x_hat_0_norm_whitened_shift.unsqueeze(-1) * (Lambda_v_std + epsilon)
        x_hat_0_norm = x_hat_0_norm_shift + mu_norm.unsqueeze(-1)

        x_rot_norm = batched_complex_hadamard(exp_f, x_hat_0_norm) # Map back to individual t frames

#         x_out = self.W_p(x_rot_norm.squeeze(-1)) # Rotate back
        x_out = x_rot_norm.squeeze(-1)
                         
        return x_out

def compute_lambda_std(lambda_h, lambda_Omega, lambda_Gamma, lambda_C, t_measure, args):
    """
    
    """

    exp_f = complex_exp(lambda_h * t_measure.unsqueeze(0).unsqueeze(2).unsqueeze(3))
    exp_b = complex_exp(-lambda_h * t_measure.unsqueeze(0).unsqueeze(2).unsqueeze(3))
    exp_c = torch.stack((torch.ones_like(exp_f[0]), torch.zeros_like(exp_f[1]))).to(lambda_h.device)
    
    K_exp_f = torch.concatenate((exp_f.flip(dims=[1])[:,:-1,:,:], exp_c), dim=1)
    K_exp_b = torch.concatenate((exp_b.flip(dims=[1])[:,:-1,:,:], exp_c), dim=1) 

    K_cov = compute_covariance_kernel(lambda_h, lambda_Omega, lambda_Gamma, K_exp_b, t_measure, args, lambda_C=lambda_C)
    K_cov = torch.flip(K_cov[0:args.seq_len], dims=(0,))
    
    Lambda_v_std = torch.sqrt(K_cov).unsqueeze(0).unsqueeze(1).unsqueeze(-1)
        
    return exp_f, exp_b, Lambda_v_std

# def dynamic_whiten(x_hat_0, epsilon):
...
#     return x_hat_0_whitened

# def dynamic_unwhiten(x_hat_0_norm_whitened, epsilon):
